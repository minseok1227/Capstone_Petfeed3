...
while True:
    if ((today10to9am < now1) & (now1 < today10past10am)) or ((today10to12pm < now1) & (now1 < today10past1pm)) or (
            (today10to5pm < now1) & (now1 < today10past6pm)):
                    ...
                    if sec == 30:
                        ...
                        def home():
                            if result.is_valid() & (result.temperature < 30) & (result.humidity < 70):
                                return render_template('index2.html', today=d, breakfast=breakfastDone, lunch=1000,
                                                       dinner=1000, temp1=result.temperature,
                            ...
                            if not result.is_valid():
                                return render_template('index2.html', today=d, breakfast=breakfastDone, lunch=1000,
                                                       dinner=1000, temp1="cannot inspect",
                ...
                while True:
                    ...
                    if sec == 30:
                        ...
                        def home():
                            if result.is_valid() & (result.temperature < 30) & (result.humidity < 70):
                                return render_template('index2.html', today=d, breakfast=breakfastDone, lunch=lunchDone,
                                                       dinner=dinnerDone, temp1=result.temperature,
                                                       humid1=result.humidity, OkorNot="O")
                            if result.is_valid() & ((result.temperature >= 30) or (result.humidity >= 70)):
                                return render_template('index2.html', today=d, breakfast=breakfastDone, lunch=lunchDone,
                                                       dinner=dinnerDone, temp1=result.temperature,
                            ...
                            if not result.is_valid():
                                return render_template('index2.html', today=d, breakfast=breakfastDone, lunch=lunchDone,
                                                       dinner=dinnerDone, temp1="cannot inspect",
                                                       humid1="cannot inspect", OkorNot="X")
